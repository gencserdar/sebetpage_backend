    ...
    public SearchResponseDTO search(String keyword) {
        ...
        List<SearchResultDTO> userResults = users.stream()
                ...
                .map(u -> {
                    ...
                    return new SearchResultDTO(u.getId(), u.getName(), u.getSurname(), u.getNickname(), "USER", mutualCount, u.getProfileImageUrl());
                })
        ...
        List<SearchResultDTO> groupResults = groups.stream()
                .map(g -> {
                    int mutualCount = (int) g.getMembers().stream()
                            ...
                            .count();
                    return new SearchResultDTO(g.getId(), g.getName(), null, null, "GROUP", mutualCount,null);
                })
                .collect(Collectors.toList());
