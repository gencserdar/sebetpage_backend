...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    public RateLimitFilter(
            ...
            @Value("${app.rate-limit.group-write.capacity}") long groupWriteCapacity,
            @Value("${app.rate-limit.group-write.window-seconds}") long groupWriteWindowSeconds,
            @Value("${app.rate-limit.group-photo.capacity}") long groupPhotoCapacity,
            @Value("${app.rate-limit.group-photo.window-seconds}") long groupPhotoWindowSeconds,
        ...
        this.rules = List.of(
                ...
                regex("group-photo",   HttpMethod.POST, "^/api/messaging-groups/[^/]+/photo$",
                        groupPhotoCapacity,      groupPhotoWindowSeconds),
                exact("group-write",   HttpMethod.POST, "/api/messaging-groups",
                        groupWriteCapacity,      groupWriteWindowSeconds),
                regex("group-write",   HttpMethod.POST, "^/api/messaging-groups/[^/]+/members$",
                        groupWriteCapacity,      groupWriteWindowSeconds),
                regex("group-write",   HttpMethod.PATCH, "^/api/messaging-groups/[^/]+$",
                        groupWriteCapacity,      groupWriteWindowSeconds),
                regex("group-write",   HttpMethod.PATCH, "^/api/messaging-groups/[^/]+/participants/[^/]+$",
                        groupWriteCapacity,      groupWriteWindowSeconds),
                regex("group-write",   HttpMethod.DELETE, "^/api/messaging-groups/[^/]+(/members/[^/]+)?$",
                        groupWriteCapacity,      groupWriteWindowSeconds),
